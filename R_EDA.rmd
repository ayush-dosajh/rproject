...
###for most part japan's Emmision due to  energy production is 1000 which is far lower to China which Goes as far as about 9,000 Mn tonnes !!!!

```{r echo = FALSE}
qplot(x= Energy , y = ..count.. / sum(..count..) , data = ghg1 , xlab = 'Energy Emission' , ylab = 'count density'  , geom = 'freqpoly' , color = Country ) +
...
ghgx<-ghg
ghgx$year_bucket <- cut (ghg$Year , c(1990 ,2000 , 2012 ))
table(ghgx$year_bucket, useNA = 'ifany')
```

###the points disply huge disparity b/w some of the observation  but the genral trend is exponential increse of industrial emminsion in comparison with Agrical emminsion over the years.

```{r echo=FALSE}
ggplot(aes(x= Agriculture , y= Industrial) ,
       data = subset(ghgx , !is.na(year_bucket))) +
  geom_point(aes(color = year_bucket) )

ggplot(aes(x= TotalGHG , y= Country) ,
       data = subset(ghgx , !is.na(year_bucket) & ghg$Country == 'India' | ghg$Country == 'China' | ghg$Country == 'Japan')) +
...
ghg_heat_map <-subset(ghg , select = c( "Energy" , "Industrial" ,"Agriculture","Waste" ,             "Land.Use" , "Electricity.Heat" ,"Manufacturing" , "Transportation" , "FugitiveEmissions" ))
...
This plot groups data for some of the countries and compares thier Total GHG emmision this plot shows the huge disparitries of emmision b/w China and others What china used to produce in the past 1990-2010 is what Developing Country like India is producing in 2000-2010 .()
